...
void RenderSystem::render()
{
		...
		// Position
		// 
		std::vector<entid_t> renderables = getAllEntities(CF_RENDERABLE);
		
		// Draw entities
		// One additional optimization would be to draw same->models sequentially to skip some binds
		for(int i = 0; i < renderables.size(); ++i)
		{
			const Position* p = getComponent(renderables[i], CT_POSITION);
			const Renderable* r = getComponent(renderables[i], CT_RENDERABLE);
